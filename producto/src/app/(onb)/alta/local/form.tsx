...
export function LocalForm({ initial }: { initial: L }) {
  ...
  // React vacía el formulario tras cada envío: si hay errores, se vuelve a mostrar lo que el usuario escribió.
    ...
    <form action={act} noValidate style={{ display: "contents" }}>
      <OnbShell step="local" back="/alta/briefing" foot={
        ...
        <div className="fgrid fgrid-2">
          <TextField label="Nombre del local" name="name" defaultValue={v?.name ?? initial.name} error={e?.name} autoComplete="organization" />
          <TextField label="Dirección" name="address" defaultValue={v?.address ?? initial.address} autoComplete="street-address" placeholder="Calle y número" />
          <TextField label="Código postal" name="postal_code" defaultValue={v?.postal_code ?? initial.postal_code} autoComplete="postal-code" inputMode="numeric" hint="Sirve para comparar precios con cocinas de tu zona, siempre de forma anónima." />
          <TextField label="Ciudad" name="ciudad" defaultValue={v?.ciudad ?? initial.ciudad} autoComplete="address-level2" />
        </div>
        <div className="q">
        ...
        <div className="fgrid fgrid-2">
          <TextField label="Food cost objetivo (%)" name="fc_objetivo" inputMode="decimal" defaultValue={v?.fc_objetivo ?? String(initial.fc_objetivo).replace(".", ",")} error={e?.fc_objetivo} hint="El coste de la materia prima sobre el precio de venta sin IVA. Lo habitual está entre el 28 y el 32 %." />
          <TextField label="Comensales al día (opcional)" name="comensales_dia" inputMode="numeric" defaultValue={v?.comensales_dia ?? initial.comensales_dia ?? ""} error={e?.comensales_dia} hint="Para calcular el ticket medio por comensal." />
        </div>
