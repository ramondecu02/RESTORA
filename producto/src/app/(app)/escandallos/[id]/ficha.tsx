...
import { costeBase, estadoFC, foodCost, neto, pvpParaFc, recetaCost, revMargen, revPvp, type ArtCost, type CostContext, type RecetaNode } from "@/lib/costing";
import { eur, pct, plural, qty } from "@/lib/format";
import { FAMILIAS } from "@/lib/briefing";
import { defaultLineUnit, lineUnitsFor, toBase, type BaseUnit, type LineUnit } from "@/lib/units";
import { archivarReceta, asegurarArticulo, duplicarReceta, guardarReceta } from "../actions";

export type FichaArt = ArtCost & { categoryId: string; aliases: string[] };
export type FichaRec = RecetaNode & { familia: string };
type Line = { key: string; articuloId: string | null; subrecetaId: string | null; cantidad: number; unidad: LineUnit };
type D = {
  ...
  lineas: Line[]; precios: Record<string, { precio: number | null; rend: number }>;
};
...
export function Ficha({ id, tipo, reventa, initial, fotoUrl, arts: initArts, recetas, catalog, cats, iva, fcLocal, canPrecios, canArts }: {
  id: string; tipo: "plato" | "elaboracion" | "menu"; reventa: boolean; initial: Omit<D, "precios" | "lineas"> & { lineas: Omit<Line, "key">[] }; fotoUrl: string | null;
  arts: FichaArt[]; recetas: FichaRec[]; catalog: { id: string; name: string; unit: BaseUnit; categoryId: string; aliases: string[] }[];
  cats: { id: string; name: string }[]; iva: number; fcLocal: number; canPrecios: boolean; canArts: boolean;
}) {
  const router = useRouter();
  const base = useMemo<D>(() => ({ ...initial, lineas: initial.lineas.map((l) => ({ ...l, key: k() })), precios: {} }), [initial]);
  const [d, setD] = useState<D>(base);
  const [arts, setArts] = useState(initArts);
  const [prevBase, setPrevBase] = useState(base);
  if (prevBase !== base) { setPrevBase(base); setD(base); }
  ...
  const dirty = strip(d) !== strip(base);
  useEffect(() => {
    if (!dirty) return;
  ...
  const ctxFor = (x: D, withPrices: boolean): CostContext => {
    ...
    for (const a of arts) {
      ...
      a2.set(a.id, p ? { ...a, rend: p.rend } : a);
      if (p && p.precio != null) overrides.set(a.id, p.precio);
    }
    ...
    r2.set(id, { id, tipo, name: x.name, raciones: x.raciones, rinde: x.rinde, rindeUnit: x.rindeUnit, reventa, costeManual: x.costeManual,
      lineas: x.lineas.map((l) => ({ articuloId: l.articuloId, subrecetaId: l.subrecetaId, cantidad: toBase(l.cantidad, l.unidad), unidad: l.unidad })) });
    return { arts: a2, recetas: r2, overrides };
  };
  const cNow = recetaCost(id, ctxFor(d, true));
  const cBase = recetaCost(id, ctxFor(base, false));
  ...
  const lineUnitBase = (l: Line): BaseUnit => (l.articuloId ? artById.get(l.articuloId)?.unit ?? "kg" : recById.get(l.subrecetaId!)?.tipo === "elaboracion" ? recById.get(l.subrecetaId!)!.rindeUnit : "ud");

  const onPick = (p: Picked) => start(async () => {
    let artId: string | null = null, subId: string | null = null, unit: BaseUnit = p.unit;
    if (p.tipo === "catalogo") {
      const r = await asegurarArticulo(p.id);
      if (!r.ok) { toastError(r.error); return; }
      const a = r.data!;
      artId = a.id; unit = a.unit;
      if (!artById.has(a.id)) setArts((xs) => [...xs, { id: a.id, name: a.name, unit: a.unit, rend: a.rend, pmp: null, lastPrice: null, lastPurchaseAt: null, precioManual: null, precioManualAt: null, categoryId: "", aliases: [] }]);
    } else if (p.tipo === "tuyo") artId = p.id;
    else subId = p.id;
    const lu: LineUnit = subId && recById.get(subId)?.tipo !== "elaboracion" ? "ud" : defaultLineUnit(unit);
    setD((x) => ({ ...x, lineas: [...x.lineas, { key: k(), articuloId: artId, subrecetaId: subId, cantidad: defQty(lu), unidad: lu }] }));
    setPicker(false);
  });

  const accept = () => start(async () => {
    const precios = Object.entries(d.precios).map(([articuloId, v]) => ({ articuloId, ...v }));
    const r = await guardarReceta(id, { ...d, lineas: d.lineas.map(({ key: _k, ...l }) => l), precios });
    if (r.ok) { toast(r.msg ?? "Guardado"); router.refresh(); } else toastError(r.error);
  });

  // Valoración del cambio (antes → después)
  const pvpNetB = base.pvp ? neto(base.pvp, iva) : null, pvpNetD = d.pvp ? neto(d.pvp, iva) : null;
  const mB = pvpNetB != null ? pvpNetB - cBase.perUnit : null, mD = pvpNetD != null ? pvpNetD - coste : null;
  const fcB = foodCost(cBase.perUnit, base.pvp, iva);
  ...
  const lineRow = (l: Line) => {
    const art = l.articuloId ? artById.get(l.articuloId) : null;
    ...
    const pr = l.articuloId ? d.precios[l.articuloId] : undefined;
    ...
    return (
      <div className="er" key={l.key}>
        <div className="er-n">
          <b>{name}{sub ? <span className="tag tag-line">{sub.tipo === "elaboracion" ? "Elaboración" : "Plato"}</span> : null}</b>
          <small>{art ? (lc?.unitCost != null ? `${eur(lc.unitCost)}/${art.unit} neto${(pr?.rend ?? art.rend) < 100 ? ` · aprovechable ${qty(pr?.rend ?? art.rend)} %` : ""}` : "Sin precio: sube un albarán que lo incluya o pon un precio a mano")
        ...
        <div className="er-q">
          <NumInput className="inp" value={l.cantidad} onValue={(n) => setLine(l.key, { cantidad: n ?? 0 })} aria-label={`Cantidad de ${name}`} />
          {units.length > 1 ? <select className="inp" value={l.unidad} aria-label="Unidad" onChange={(e) => {
            const nu = e.target.value as LineUnit;
            setLine(l.key, { unidad: nu, cantidad: Math.round((toBase(l.cantidad, l.unidad) / toBase(1, nu)) * 1000) / 1000 });
          }}>{units.map((u) => <option key={u}>{u}</option>)}</select> : <span>{units[0]}</span>}
        </div>
        <div className="er-c">{eur(lc?.cost ?? null)}</div>
        <button type="button" className="iconbtn er-x" aria-label={`Quitar ${name}`} onClick={() => setD((x) => ({ ...x, lineas: x.lineas.filter((y) => y.key !== l.key) }))}><Icon name="close" /></button>
        {art && open[l.key] ? (
          <div className="er-price">
            <label className="row">Compra <NumInput className="inp" decimals={4} value={precioCompra} disabled={!canArts} onValue={(n) => setD((x) => ({ ...x, precios: { ...x.precios, [art.id]: { precio: n, rend: x.precios[art.id]?.rend ?? art.rend } } }))} aria-label={`Precio de compra de ${name}`} /> €/{art.unit}</label>
            <label className="row">Aprovechable <NumInput className="inp" decimals={1} value={pr?.rend ?? art.rend} disabled={!canArts} onValue={(n) => setD((x) => ({ ...x, precios: { ...x.precios, [art.id]: { precio: x.precios[art.id]?.precio ?? null, rend: Math.min(100, Math.max(1, n ?? 100)) } } }))} aria-label={`Aprovechable de ${name}`} /> %</label>
            {pr ? <span className="tag tag-warn">Cambia en todas tus recetas al aceptar</span> : null}
          </div>
        ) : null}
        {sub && open[l.key] && subC ? (
  ...
  const foot = (
    <div className="foot-in">
      ...
      <div className="foot-btns">
        <button type="button" className="btn btn-3" disabled={!dirty || pending} onClick={() => { setD(base); toast("Cambios descartados"); }}>Descartar</button>
        ...
        <div className="esc-col">
            ...
            <div className="esc-meta">
              ...
              {tipo === "elaboracion" ? (
                <div className="fld"><label htmlFor="r-ri">Rinde</label><div className="inp-unit"><NumInput id="r-ri" className="inp" value={d.rinde} onValue={(n) => set("rinde", n && n > 0 ? n : 1)} />
                  <select className="inp" style={{ width: 80 }} value={d.rindeUnit} onChange={(e) => set("rindeUnit", e.target.value as BaseUnit)} aria-label="Unidad de rinde"><option>kg</option><option>L</option><option>ud</option></select></div></div>
              ) : !reventa ? (
              ...
              <div className="fgrid fgrid-3">
                <div className="fld"><label htmlFor="rv-c">Precio de compra (€)</label>
                  <NumInput id="rv-c" decimals={4} value={d.lineas.length ? coste : d.costeManual} disabled={!!d.lineas.length || !canPrecios}
                    onValue={(n) => setD((x) => ({ ...x, costeManual: n, pvp: n != null && x.margenObjetivo != null ? Math.round(revPvp(n, x.margenObjetivo, iva) * 100) / 100 : x.pvp }))} />
                ...
                <div className="fld"><label htmlFor="rv-m">Margen deseado (%)</label>
                  <NumInput id="rv-m" decimals={1} value={d.margenObjetivo} disabled={!canPrecios}
                    onValue={(n) => setD((x) => ({ ...x, margenObjetivo: n == null ? null : Math.min(99, Math.max(0, n)), pvp: n != null ? Math.round(revPvp(revCoste, Math.min(99, Math.max(0, n)), iva) * 100) / 100 : x.pvp }))} /></div>
                <div className="fld"><label htmlFor="rv-p">Precio de venta (€, con IVA)</label>
                  <NumInput id="rv-p" decimals={2} value={d.pvp} disabled={!canPrecios}
                    onValue={(n) => setD((x) => ({ ...x, pvp: n, margenObjetivo: n ? Math.round(revMargen(revCoste, n, iva)) : x.margenObjetivo }))} /></div>
              </div>
          ...
          <section className="card" aria-labelledby="h-mas">
            ...
            <div className="row-wrap">
              <button type="button" className="btn btn-3 btn-sm" onClick={() => start(() => duplicarReceta(id))}><Icon name="copy" size={18} /> Duplicar</button>
              <button type="button" className="btn btn-3 btn-sm" onClick={() => setAsk("borrar")}><Icon name="trash" size={18} /> Borrar</button>
            </div>
          </section>
        </div>
            ...
            {tipo !== "elaboracion" ? <>
              {!reventa ? <div className="fld"><span className="lbl" id="l-fc">Food cost objetivo</span>
                <div className="pills pills-5" role="radiogroup" aria-labelledby="l-fc">{[25, 28, 30, 33, 35].map((v) => (
                  <button key={v} type="button" role="radio" aria-checked={fcObj === v} className={`pill ${fcObj === v ? "is-on" : ""}`} onClick={() => set("fcObjetivo", v === fcLocal ? null : v)}>{v} %</button>))}</div></div> : null}
              {!reventa ? <div className="pvp"><span className="eyebrow">PVP para un food cost del {fcObj} %</span><b>{eur(obj.gross)}</b><small>{eur(obj.net)} + IVA {iva} %</small></div> : null}
              ...
              <div className={`fcr fcr-${est.estado === "none" ? "none" : est.estado === "crit" ? "bad" : est.estado}`}>
                ...
                <span>{est.estado === "none" ? "Escribe tu precio de carta y te decimos el food cost real y el margen." :
                  <>Food cost del <b>{pct(fc)}</b>{est.estado === "ok" ? `: dentro de tu objetivo del ${fcObj} %.` : `: por encima de tu objetivo del ${fcObj} %.`} {d.pvp && d.pvp >= obj.gross ? `Te sobran ${eur(d.pvp - obj.gross)} sobre el PVP objetivo.` : d.pvp ? `Para cumplirlo, el PVP debería ser ${eur(obj.gross)} (${eur(obj.gross - d.pvp)} más).` : ""}</>}</span>
              </div>
              <div className="fld"><label htmlFor="r-v">Unidades vendidas al mes</label><NumInput id="r-v" decimals={0} value={d.ventasMes} onValue={(n) => set("ventasMes", Math.max(0, Math.round(n ?? 0)))} /><p className="hint">Se actualiza al importar ventas; también puedes ponerlo a mano.</p></div>
            </> : null}
          </section>
          {valor ? (
            <section className="card" aria-labelledby="h-val">
              ...
              {dirty && mB != null && mD != null ? <dl className="cfg">
                ...
                {d.ventasMes ? <div><dt>Aporta al mes</dt><dd>{eur(mB * base.ventasMes)} → {eur(mD * d.ventasMes)}</dd></div> : null}
              </dl> : null}
              <div className={`fcr ${valor.cls}`}><Icon name={valor.cls === "fcr-ok" ? "check" : valor.cls === "fcr-bad" ? "alert" : "info"} /><span>{valor.txt}</span></div>
              <div className="row-wrap"><button type="button" className="btn btn-sm" disabled={!dirty || pending} onClick={accept}>Aceptar cambios</button><button type="button" className="btn btn-2 btn-sm" disabled={!dirty || pending} onClick={() => { setD(base); toast("Cambios descartados"); }}>Descartar</button></div>
            </section>
          ) : null}
          {breakdown.length ? (
