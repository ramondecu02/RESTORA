...
export default async function Elaboraciones() {
  ...
  const data = await withTenant(ctx.tenantId, async (c) => {
    ...
    const usos = await all<{ subreceta_id: string; n: number }>(c, "select subreceta_id, count(distinct receta_id)::int as n from receta_lineas where subreceta_id is not null group by 1");
        ...
        {elabs.length ? <div className="list">{elabs.map((r) => {
          ...
          return (
            <Link key={r.id} className="li" href={`/escandallos/${r.id}`}>
              ...
              <span className="li-end"><b>{eur(c.perUnit)}/{r.rinde_unit}</b><small>{data.usos.get(r.id) ? `en ${plural(data.usos.get(r.id)!, "plato", "platos")}` : "sin usar"}</small></span>
            </Link>);
