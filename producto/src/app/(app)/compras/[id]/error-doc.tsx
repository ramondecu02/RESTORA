...
export function ErrorDoc({ id, error, canRetry }: { id: string; error: string | null; canRetry: boolean }) {
  ...
  return (
    <TaskScreen title="No hemos podido leerlo" back="/compras">
      <div className="card"><div className="empty">
        ...
        <div className="empty-actions">
          ...
          <button type="button" className="btn btn-2" disabled={pending} onClick={() => go(() => aMano(id))}><Icon name="edit" size={18} /> Meterlo a mano</button>
          <button type="button" className="btn btn-3" disabled={pending} onClick={() => go(() => descartar(id))}>Descartar</button>
        </div>
      </div></div>
    </TaskScreen>
  );
