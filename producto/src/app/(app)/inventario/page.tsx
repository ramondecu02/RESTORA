...
export default async function Inventario() {
  ...
  const data = await withTenant(ctx.tenantId, async (c) => {
    ...
    const pedidos = (await one<{ n: number }>(c, "select count(*)::int as n from pedidos where local_id = $1 and estado in ('borrador','enviado')", [ctx.local.id]))?.n ?? 0;
    return { arts, pedidos };
  });
  ...
  const untracked = data.arts.filter((a) => !a.track_stock).map((a) => ({ id: a.id, name: a.name, unit: a.unit, categoryId: a.category_id, aliases: a.aliases }));
  return (
