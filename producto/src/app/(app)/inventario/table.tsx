...
import { barraCobertura, cantidadPedido, coberturaDias, estadoStock } from "@/lib/inventory";
...
export function InvTable({ rows: initial, untracked, catalog, cats, local, pedidos }: {
  rows: Row[]; untracked: { id: string; name: string; unit: BaseUnit; categoryId: string; aliases: string[] }[];
  ...
  const [addVals, setAddVals] = useState({ stock: 0 as number | null, minimo: 2 as number | null, consumo: 2 as number | null });
  ...
  const cobMedia = calc.length ? calc.reduce((s, r) => s + Math.min(r.dias, 99), 0) / calc.length : 0;
  ...
  const commitStock = (r: Row) => start(async () => { const res = await ajustarStock(r.id, rows.find((x) => x.id === r.id)!.stock); if (!res.ok) toastError(res.error); });
  const commitParam = (r: Row, campo: "stock_min" | "consumo_semanal") => start(async () => {
    const cur = rows.find((x) => x.id === r.id)!;
    const res = await guardarParametro(r.id, campo, campo === "stock_min" ? cur.minimo : cur.consumo);
  ...
  const statRow = (
    <div className="stats">
      ...
      <div className="stat"><span className="stat-k">Cobertura media</span><span className="stat-v">{qty(cobMedia, 1)} d</span><span className="stat-s">Al ritmo actual</span></div>
    </div>
  );
  ...
  return (
    <>
              ...
              {shown.map((r, i) => {
                ...
                return [
                  ...
                  <tr key={r.id}>
                    ...
                    <td><button type="button" className="iconbtn iconbtn-sm iconbtn-danger" aria-label={`Quitar ${r.name} del inventario`} onClick={() => start(async () => { const res = await dejarDeControlar(r.id); if (res.ok) toast(res.msg ?? "Quitado"); else toastError(res.error); })}><Icon name="close" size={16} /></button></td>
                  </tr>,
                ];
              })}
            </tbody>
      ...
      <Sheet open={pedidoOpen} onClose={() => setPedidoOpen(false)} title="Pedido sugerido" sub="Para lo que está bajo mínimo: hasta el doble del mínimo" wide
        foot={<>
          <button type="button" className="btn btn-3" onClick={() => setPedidoOpen(false)}>Cerrar</button>
          <button type="button" className="btn btn-2" disabled={pending || !pedLineas().length} onClick={() => start(async () => { await pedidoAFactura(null, pedLineas()); })}>Registrar como factura</button>
          ...
          <section key={g.name} className="card">
            ...
            <div className="list">{g.rows.map((r) => (
              <div className="li" key={r.id}>
                <span className="li-main"><b>{r.name}</b><small>Tienes {qty(r.stock)} · mínimo {qty(r.minimo)} {r.unit}</small></span>
        ...
        {!porProv.length ? <p className="muted">No hay nada bajo mínimo.</p> : null}
        <div className="mini-sum"><span>Total estimado</span><b>{eur(calc.reduce((s, r) => s + (ped[r.id] ?? 0) * (r.precio ?? 0), 0))}</b></div>
        <p className="hint">RESTORA no envía pedidos por ti: prepara el texto y lo mandas tú. «Recibir pedido» suma el stock sin tocar precios; «Registrar como factura» abre una compra con estas líneas para ajustar precios reales.</p>
      </Sheet>

      <ArticlePicker open={addOpen && !adding} onClose={() => setAddOpen(false)} title="Añadir al inventario"
        arts={untracked} catalog={catalog} cats={catNames} onPick={(p) => { if (p.tipo !== "receta") setAdding(p); }} />
      <Sheet open={addOpen && !!adding} onClose={() => { setAdding(null); setAddOpen(false); }} title={adding?.name ?? ""} sub="Valores de partida: los puedes cambiar luego"
        foot={<><button type="button" className="btn btn-3" onClick={() => setAdding(null)}>Atrás</button>
          <button type="button" className="btn" disabled={pending} onClick={() => start(async () => {
            const r = await anadirReferencia({ tipo: adding!.tipo as "tuyo" | "catalogo", id: adding!.id, ...addVals });
        ...
        <div className="fgrid fgrid-3">
          <div className="fld"><label htmlFor="ad-s">Stock actual ({adding?.unit})</label><NumInput id="ad-s" value={addVals.stock} onValue={(n) => setAddVals({ ...addVals, stock: n })} /></div>
      ...
      <Sheet open={!!salida} onClose={() => setSalida(null)} title="Registrar merma o salida" sub="Resta del stock lo que no se ha vendido"
        foot={<><button type="button" className="btn btn-3" onClick={() => setSalida(null)}>Cancelar</button>
          <button type="button" className="btn" disabled={pending || !(salida?.cantidad && salida.cantidad > 0)} onClick={() => start(async () => {
            ...
            if (r.ok) { toast(r.msg ?? "Registrado"); const q = salida!.cantidad!; upd(salida!.id, { stock: (rows.find((x) => x.id === salida!.id)?.stock ?? 0) - q }); setSalida(null); } else toastError(r.error);
          })}>Registrar</button></>}>
